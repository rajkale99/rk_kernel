#!/bin/bash
echo Welcome To Automated Kernel Compiler For smartron
echo By rajkale99
make clean && make mrproper && rm -rf output && rm -rf dsm/modules && rm -rf dsm/zImage
echo Cleaned Some Old Crappy Files
export CROSS_COMPILE=~/toolchains/ubertc4.9/bin/aarch64-linux-android-
mkdir output && mkdir modules && mkdir -p output/modules
echo Compiler Set Sucessful
export ARCH=arm64
echo Compiler Set To Build For ARM64
make -C $(pwd) O=output rk_defconfig
echo Picked Stock Config
make -j16 -C $(pwd) O=output
cp ~/rkoreo/output/arch/arm64/boot/Image.gz-dtb ~/rkoreo/dsm
mv ~/rkoreo/dsm/Image.gz-dtb ~/rkoreo/dsm/zImage
cd ~/rkoreo/output && mkdir -p modules/
find ~/rkoreo/output -name '*.ko' -exec cp {} modules/ \;
cd ~/rkoreo/output/modules
~/toolchains/ubertc4.9/bin/aarch64-linux-android-strip --strip-unneeded *.ko && mkdir pronto && cp -a wlan.ko pronto_wlan.ko && mv pronto_wlan.ko pronto
mv ~/rkoreo/output/modules/ ~/rkoreo/dsm
echo Building Has Been Completed
