...
cp -rf ~/r/output/arch/arm64/boot/Image.gz-dtb ~/ank/
cd ~/ank/
mv Image.gz-dtb zImage
#c#d ~/r/output && mkdir -p modules/
#find ~/rkoreo/output -name '*.ko' -exec cp {} modules/ \;
#cd ~/rkoreo/output/modules
#~/toolchains//bin/aarch64-linux-android-strip --strip-unneeded *.ko && mkdir pronto && cp -a wlan.ko pronto_wlan.ko && mv pronto_wlan.ko pronto
